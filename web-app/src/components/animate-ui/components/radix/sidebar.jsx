...
function Sidebar({
  ...
  const width = isCollapsed 
    ? (isIcon ? (variant === 'floating' || variant === 'inset' ? `calc(${SIDEBAR_WIDTH_ICON} + 1rem)` : SIDEBAR_WIDTH_ICON) : '0px')
    : SIDEBAR_WIDTH;
  ...
  return (
    <div
      ...
      <motion.div
        ...
        animate={{ 
          width,
          left: isCollapsed && isOffcanvas ? 'calc(var(--sidebar-width) * -1)' : '0px',
        }}
        transition={transition}
        className={cn(
          'fixed inset-y-0 z-10 hidden h-svh transition-none md:flex',
        ...
        <Highlight
          ...
          <div
            ...
            className="bg-inherit group-data-[variant=floating]:border-sidebar-border flex h-full w-full flex-col group-data-[variant=floating]:rounded-lg group-data-[variant=floating]:border group-data-[variant=floating]:shadow-sm overflow-hidden">
            {children}
          </div>
        </Highlight>
      </motion.div>
  ...
  return (
    <Comp
      ...
      animate={{ 
        ...
        height: isCollapsed ? 0 : '2rem',
        marginBottom: isCollapsed ? 0 : '0.5rem'
      }}
      className={cn(
        'text-sidebar-foreground ring-sidebar-ring flex shrink-0 items-center rounded-md px-2 text-xs font-medium outline-hidden transition-colors focus-visible:ring-2 [&>svg]:size-4 [&>svg]:shrink-0 overflow-hidden',
        className
      )}
      {...props} />
  );
