...
const StageMobile = ({ containerRef }) => {
  ...
  const headingX = useTransform(scrollXProgress, [0.25, 0.4], [isMobile ? 0 : 50, 0]);
  const headingY = useTransform(scrollXProgress, [0.25, 0.4], [isMobile ? 20 : 0, 0]);

  const textOpacity = useTransform(scrollXProgress, [0.3, 0.45], [0, 1]);
  const textX = useTransform(scrollXProgress, [0.3, 0.45], [isMobile ? 0 : 50, 0]);
  const textY = useTransform(scrollXProgress, [0.3, 0.45], [isMobile ? 20 : 0, 0]);
  ...
  const fanProgress = useTransform(scrollXProgress, [0.2, 0.5], [0, 1]);
    ...
    <section ref={targetRef} className="w-screen h-full flex items-center justify-center p-6 md:p-20 bg-transparent shrink-0">
      <motion.div style={{ opacity }} className="max-w-7xl w-full grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-24 items-center">
        ...
        <div className="space-y-6 md:space-y-10 order-1 lg:order-2 text-center lg:text-left flex flex-col items-center lg:items-start">
          ...
          <motion.p style={{ opacity: textOpacity, x: textX, y: textY }} className="text-on-surface-variant text-sm md:text-xl leading-relaxed max-w-lg">
