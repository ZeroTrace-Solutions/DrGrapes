...
const StageGear = ({ containerRef }) => {
  ...
  return (
    <section ref={targetRef} className="w-screen h-full flex items-center justify-start p-20 bg-transparent">
      <motion.div style={{ opacity, x }} className="flex gap-12 pl-20">
        <div className="w-[400px] flex flex-col justify-center space-y-6">
          <h2 className="text-7xl font-black uppercase leading-[0.9] tracking-tighter">
            The <br/>
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-primary-container">Bestseller</span> <br/>
            Series
          </h2>
          <p className="text-on-surface-variant text-body-md">Swipe through our most requested professional equipment.</p>
        ...
        <div className="flex gap-8">
          {[
            { name: "Elite Stethoscope", price: "249", img: eliteStethoscope, color: "from-secondary/30" },
            { name: "Anatomy Atlas", price: "120", img: anatomyAtlas, color: "from-primary/30" },
            { name: "Diagnostic Kit", price: "85", img: reflexHammer, color: "from-secondary-container/30" }
          ].map((product, i) => (
            <motion.div 
              key={i}
            ...
            >
              ...
              <div className="relative z-10 h-full flex flex-col justify-between">
                ...
                <button className="w-full py-4 bg-primary-container text-on-primary hover:bg-primary rounded-2xl transition-all font-black flex items-center justify-center gap-2 shadow-lg shadow-primary-container/20">
                  View Gear <Plus className="w-4 h-4" />
                </button>
              </div>
            </motion.div>
          ))}
